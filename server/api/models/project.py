...
class Project(BaseModel):
    ...
    def get_app_script(self, new=False, query=None) -> Optional[str]:
        ...
        app_url = (
            f"{os.getenv('ORIGIN_PROTOCOL')}://{os.getenv('ORIGIN_DOMAIN')}/server/{container_session.port}/{self.id}"
        )
    ...
    def wait_for_server_to_be_ready(self, container_session: ContainerSessionBase):
        ...
        url = f"http://localhost:{container_session.port}/server/{container_session.port}/{self.id}"
